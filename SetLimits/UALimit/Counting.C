...
void MakeWorkspace( void ){
 ...
 // observable: number of events
  pWs->factory( "n[0]" ); // NOTE: this is how you create a variable/parameter/observable

  // To defined the signal events as u_sig = Lumi.Eff. Acceptance

  // integrated luminosity
//  pWs->factory( "lumi[0]" );

// Lumi With systematcis
...
// Also adjust Efficiency
 ...
 // signal yield
  ...
  // signal yield
// pWs->factory( "nsig[0,0,100]" ); // NOTE: three parameters are "current value", "low bound", "upper bound"

  // background yield
//  pWs->factory( "nbkg[10,0,100]" );


    //Modifying  background yield to take into account systematics
//  pWs->factory( "nbkg_nom[10]" );
//  pWs->factory( "prod::nbkg(nbkg_nom,alpha_lumi)" );

 // background yield with systematics of 10%
  ...
  // full event yield
  pWs->factory( "sum::yield(nsig,nbkg)" ); // NOTE: lower-case "sum" create a function. Upper-case "SUM" would create a PDF

 // define Bayesian prior PDF for POI
  pWs->factory( "Uniform::prior(xsec)" );


  // No Systematics Core model: Poisson probability with mean signal+bkg
  pWs->factory( "Poisson::model_core(n,yield)" ); // NOTE: "model_core" is a name of the PDF object

  // First model with systematics
//  pWs->factory( "PROD::model(model_core,constr_lumi)" );

// Second model with systematics
//  pWs->factory( "PROD::model(model_core,constr_lumi,constr_efficiency)" );

 // Final model with systematics
