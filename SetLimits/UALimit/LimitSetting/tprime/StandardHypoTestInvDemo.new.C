...
public:
  ...
  void
  SaveInverterResult( const char * fileNameBase,
...
FreqLimitCalculator::FreqLimitCalculator() : mPlotHypoTestResult(true),
					     mWriteResult(false),
					     mOptimize(true),
...
void
FreqLimitCalculator::SetParameter(const char * name, bool value){
  //
...
void
FreqLimitCalculator::SetParameter(const char * name, int value){
  //
...
void
FreqLimitCalculator::SetParameter(const char * name, double value){
  //
...
void
FreqLimitCalculator::SetParameter(const char * name, const char * value){
  //
...
*/
  ...
  calc.SaveInverterResult( infile, r, calculatorType, testStatType, useCls, npoints );
...
void
FreqLimitCalculator::SaveInverterResult( const char * fileNameBase,
					 HypoTestInverterResult * r,
					 int calculatorType,
					 int testStatType, 
					 bool useCls,  
  ...
  if (!resultName) resultName = r->GetName();
...
HypoTestInverterResult *
FreqLimitCalculator::RunInverter(RooWorkspace * w,
					const char * modelSBName, const char * modelBName, 
			...
			const char * nuisPriorName = 0)    
{
   ...
   if (w != NULL) {
     ...
     std::cerr << "No workspace - Exit " 
	       << std::endl;
     return result; 
   }
      		
   SaveInverterResult( r, calculatorType, testStatType, useCls, npoints );
