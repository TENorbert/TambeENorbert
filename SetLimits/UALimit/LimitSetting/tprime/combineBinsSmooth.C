...
int Move2dBin( TH2 * hist, int x, int y, int xn, int yn ){
  //
  // Move bin content to another bin (add)
  // Add the corresponding bin error as well
  //
  int ibin = hist->GetBin(x,y);
  int ibin_dest = hist->GetBin(xn,yn);

  double _content = hist->GetBinContent(ibin);
  double _error = hist->GetBinError(ibin);
  double _error_dest = hist->GetBinError(ibin);

  _error_dest = sqrt(_error*_error + _error_dest*_error_dest);
  hist->AddBinContent( ibin_dest, _content );
  hist->SetBinError( ibin_dest, _error_dest );
...
int MoveOverflow( TH2 * hist ){
  ...
  for (int k=0; k<nOverflowBins; ++k){
    ...
    if (j==0){                   // walks low line
      ...
      Move2dBin(hist,i,j,i_dest,j_dest);
...
combineBins( std::string channel, int mass, double scale_factor,
  ...
  std::cout << "Integral no overflow: " << hs->Integral(1,nx,1,ny) << std::endl << std::endl;

  TH2D *hb = (TH2D*)hs->Clone(); 
  hb->SetName("hb");
  hb->Reset();
  TH2D *hX[40];
  double bEntries[40]; // array with number of entries
  for (int i=0;i<nb;i++){
  ...
  int nbin=nx*ny;
  int nbin_all=(nx+2)*(ny+2); // including overflow bins - needed for absolute numbering
  ...
  std::vector<std::vector<int> > vMap(nbin_all);
  ...
  float xsb[30000],xx[30000][40],xe[30000][40];
  int ibin; 
  double _sum = 0.0;
  ...
  for (int i=1;i<nx+1;i++){
	...
	if (xs_s>0){}
      ...
      xx[newbin][0]=xs;
      xe[newbin][0]=hs->GetBinError(ibin);
      xx[newbin][1]=xb;
      xe[newbin][1]=hb->GetBinError(ibin);
      for (int k=0;k<nb;k++){
	xx[newbin][k+2]=hX[k]->GetBinContent(ibin);
	xe[newbin][k+2]=hX[k]->GetBinError(ibin);
      }
      ...
      h1sb->SetBinContent(newbin,xsb[newbin]);
      h1s->SetBinContent(newbin,xx[newbin][0]);
      h1s->SetBinError(newbin,xe[newbin][0]);
      h1b->SetBinContent(newbin,xx[newbin][1]);
      h1b->SetBinError(newbin,xe[newbin][1]);
  ...
  std::cout << "New bins: " << newbin << std::endl;
