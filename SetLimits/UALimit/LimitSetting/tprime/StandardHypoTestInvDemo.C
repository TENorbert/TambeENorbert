...
std::vector<Double_t> StandardHypoTestInvDemo(const char * fileName =0,
                                              ...
                                              )
{
  /*
std::vector<Double_t>
StandardHypoTestInvDemo(const char * fileName = 0,
                        const char * wsName = "combined",
                        const char * modelSBName = "ModelConfig",
                        const char * modelBName = "",
                        const char * dataName = "obsData",                 
...
*/
  ...
  if (string(suffix).find("merge")!=std::string::npos){
    char buf[255];
    
    ifstream ifs(suffix); 
     if(!ifs.good()){
       std::cout<<"WARNING ! Didn't find file "<<suffix<<std::endl;
     }
     ...
     while( ifs ) {
       ...
       file2 = new TFile(buf, "read");
       if (!r){
	 std::cout << "Getting HypoTestInverterResult from " << buf << std::endl;
	 r = dynamic_cast<HypoTestInverterResult*>( file2->Get(resName.c_str()) );
       }
       else{
  ...
  */
  ...
  result = calc.AnalyzeResult( r, calculatorType, testStatType, useCLs, npoints, suffix, peak);//fileName, suffix, peak);
...
RooStats::HypoTestInvTool::RunInverter(RooWorkspace * w,
   ...
   */
   ...
   //   RooRealVar * eb = dynamic_cast<RooRealVar*>(fitres.floatParsFinal().find("epsb"));
   //   epsb[i] = eb -> getVal();
   //epsb_err[i] = eb -> getError();
   fitres->floatParsFinal().Print("v");
   //   std::cout<<eb->getVal()<<std::endl;
   std::cout<<" LGM "<<std::endl;
